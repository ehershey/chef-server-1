...
destroy () {
  ...
  terraform workspace delete "$workspace" || error "terraform failed to delete the $workspace workspace! Manual cleanup of resources may be required"
}
...
destroy-all () {
  ...
  for workspace in $(consul kv get -keys terraform/chef-server/ | sed "s/.*:\(${BUILD_NUMBER}-.*$\)/\1/;s/\///;" | sort -u); do
